...
function applyRpcFromSystem() {
  ...
  const rpcUrl = status?.network?.rpc?.[0] || (status?.chainId ? window.networkManager?.getNetworkById(parseInt(status.chainId, 16))?.rpc?.[0] : "") || window.networkManager?.getPopularNetworks?.(1)?.[0]?.rpc?.[0] || "https://bsc-testnet.publicnode.com";
  ...
  const a = document.getElementById("rpcUrlText");
  if (a) a.href = rpcUrl;
}
```
...
```html
<div data-component="shared/components/network-search.html" data-ns-placeholder="Digite o nome da rede ou Chain ID (ex: Ethereum, Polygon, 1, 137)" data-ns-min-chars="2" data-ns-show-popular="true"></div>
```
