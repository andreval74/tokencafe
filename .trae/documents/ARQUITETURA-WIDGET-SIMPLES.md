...
1. Nome do Projeto [text] 2. Blockchain [select] 3. Endereço do Contrato [text]

<!-- Campos Opcionais (Avançado) -->
4. Preço por Token [number] (auto-detect) 5. Limites de Compra [number] (auto-detect) 6. Textos Personalizados [text] (template-based)
```
...
async function autoDetectContractParams(saleContract, abi, provider) {
  ...
  const tokenFunction = abi.find((f) => f.name === "saleToken" || f.name === "token" || f.name === "getToken");
  ...
  const walletFunction = abi.find((f) => f.name === "destinationWallet" || f.name === "owner" || f.name === "getOwner");
  ...
  const priceFunction = abi.find((f) => f.name === "bnbPrice" || f.name === "price" || f.name === "getPrice");
