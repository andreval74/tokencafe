...
// Valida e retorna endereço EVM para embedding em Solidity, ou null se inválido
function safeEvmAddress(addr) {
  if (!addr) return null;
  const s = String(addr).trim();
  if (!/^0x[0-9a-fA-F]{40}$/.test(s)) return null;
  // Rejeita address(0) — não tem sentido como owner/holder
...
// 1. ERC20 Minimal
// ==========================================
function getERC20Minimal(name, symbol, decimals, totalSupply, initialHolder) {
  ...
  // Se initialHolder válido, tokens vão para essa carteira; caso contrário, para msg.sender (criador)
  const holderLit = safeEvmAddress(initialHolder) || 'msg.sender';
...
contract ${safeName} {
    ...
    constructor() {
        ...
        balanceOf[${holderLit}] = totalSupply;
        emit Transfer(address(0), ${holderLit}, totalSupply);
    }
...
// ==========================================
function getERC20Controls(name, symbol, decimals, totalSupply, initialOwner, initialHolder) {
    const safeName = sanitize(name);
    ...
    const nameLit = solidityStringLiteral(name);
    const symbolLit = solidityStringLiteral(symbol);
    // Endereço de quem recebe os tokens; padrão = msg.sender (criador)
    const holderLit = safeEvmAddress(initialHolder) || 'msg.sender';
    // Endereço do administrador do contrato; padrão = msg.sender
    const ownerAddr = safeEvmAddress(initialOwner);
    // Se initialOwner for diferente do deployer, transfere ownership no constructor
    const ownershipTransfer = ownerAddr ? `\n        transferOwnership(${ownerAddr});` : '';
...
contract ${safeName} is Ownable, Pausable {
    ...
    constructor() {
        totalSupply = ${safeSupply} * 10**decimals;
        balanceOf[${holderLit}] = totalSupply;
        emit Transfer(address(0), ${holderLit}, totalSupply);${ownershipTransfer}
    }
...
function getContractSource(type, data) {
    // data: { name, symbol, decimals, totalSupply, sale, initialOwner, initialHolder }
    const { name, symbol, decimals, totalSupply, sale, initialOwner, initialHolder } = data;

    switch (type) {
        case "erc20-controls":
            return {
                source: getERC20Controls(name, symbol, decimals, totalSupply, initialOwner, initialHolder),
        ...
        default:
            return {
                source: getERC20Minimal(name, symbol, decimals, totalSupply, initialHolder),
                contractName: sanitize(name)
            };
    }
}
