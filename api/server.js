...
app.post("/api/generate-token", async (req, res) => {
  try {
    const { name, symbol, totalSupply, decimals = 18, type = "erc20-minimal", sale = {}, initialOwner, initialHolder } = req.body;
...
app.get("/api/explorer-getsourcecode", async (req, res) => {
  try {
    ...
    const verified = (src.length > 0) || (abi.length > 2 && abi !== "Contract source code not verified");
    ...
    if (!js || status !== "1") {
      try {
        ...
        const abi2 = String(f2?.ABI || "");
        const ver2 = (src2.length > 0) || (abi2.length > 2 && abi2 !== "Contract source code not verified");
        ...
        let js = null;
         ...
         if (!js || status !== "1") {
              ...
              try {
                ...
                const abi2 = String(f2?.ABI || "");
                const ver2 = (src2.length > 0) || (abi2.length > 2 && abi2 !== "Contract source code not verified");
                return res.json({ success: true, verified: ver2, sourceCode: src2, explorer: { contractName: f2?.ContractName || "", compilerVersion: f2?.CompilerVersion || "", abi: f2?.ABI || "", sourceCode: src2, optimizationUsed: f2?.OptimizationUsed || "", runs: f2?.Runs || "", evmVersion: f2?.EVMVersion || "" } });
             } catch (_) {}
        }

        return res.json({ success: true, verified, sourceCode: src, explorer: { contractName: first?.ContractName || "", compilerVersion: first?.CompilerVersion || "", abi: first?.ABI || "", sourceCode: src, optimizationUsed: first?.OptimizationUsed || "", runs: first?.Runs || "", evmVersion: first?.EVMVersion || "" } });
    } catch (e) {
...
function loadSystemPrompt() {
  try {
    const p = path.join(__dirname, "../ai/prompts/contract-analysis.md");
    return fs.existsSync(p) ? fs.readFileSync(p, "utf8").trim() : "";
  } catch (_) { return ""; }
}

function saveAnalysisToTraining(entry) {
  try {
    const dir = path.join(__dirname, "../ai/training");
    if (!fs.existsSync(dir)) fs.mkdirSync(dir, { recursive: true });
    const file = path.join(dir, "contract-analyses.jsonl");
    fs.appendFileSync(file, JSON.stringify(entry) + "\n", "utf8");
  } catch (e) {
    console.warn("[AI Training] save failed:", e.message);
  }
}
...
async function callGroq(systemPrompt, userPrompt, apiKey) {
  ...
  const body = JSON.stringify({ model: "llama-3.3-70b-versatile", max_tokens: 500, messages });
  return new Promise((resolve, reject) => {
    ...
    const r = https.request(opts, (res) => { let raw = ""; res.on("data", c => raw += c); res.on("end", () => { try { resolve(JSON.parse(raw)); } catch (e) { reject(e); } }); });
    r.on("error", reject); r.write(body); r.end();
  });
}

async function callAnthropic(systemPrompt, userPrompt, apiKey) {
  const body = JSON.stringify({ model: "claude-haiku-4-5-20251001", max_tokens: 500, ...(systemPrompt ? { system: systemPrompt } : {}), messages: [{ role: "user", content: userPrompt }] });
  return new Promise((resolve, reject) => {
    const opts = {
...
async function fetchContractOnChainData(chainId, address) {
  const data = {};
  const apiKey = getExplorerApiKeyFromEnv();
  try {
    const info = await getContractCreationInfo(chainId, address, apiKey);
    if (info?.deployer) data.deployer = info.deployer;
    if (info?.txHash) data.creationTx = info.txHash;
  } catch (_) {}
  try {
    const base = getExplorerApiUrl(chainId);
    const qs = new URLSearchParams({ module: "account", action: "tokentx", contractaddress: address, page: "1", offset: "5", sort: "desc" });
    if (apiKey) qs.append("apikey", apiKey);
    const resp = await fetch(`${base}&${qs}`, { signal: AbortSignal.timeout(4000) });
    if (resp.ok) {
      const js = await resp.json();
      if (Array.isArray(js?.result) && js.result.length) {
        data.recentTransfers = js.result.length;
        const ts = parseInt(js.result[0]?.timeStamp || "0", 10);
        if (ts) data.lastActivity = new Date(ts * 1000).toISOString().slice(0, 10);
      }
    }
  } catch (_) {}
  return data;
...
app.post("/api/analyze-contract", async (req, res) => {
  ...
  const { sourceCode, contractName, abi, chainId, address } = req.body || {};
  if (!sourceCode && !abi) {
    return res.status(400).json({ success: false, error: "sourceCode ou abi obrigatório" });
  }

  try {
    const systemPrompt = loadSystemPrompt();
    const abiSummary = Array.isArray(abi) ? abi.filter(x => x.type === "function").map(x => x.name).join(", ") : typeof abi === "string" ? abi.slice(0, 500) : "";
    const srcSnippet = typeof sourceCode === "string" ? sourceCode.slice(0, 4000) : "";

    // Enriches prompt with on-chain data if address/chainId provided
    let onChainSection = "";
    if (address && chainId) {
      try {
        const onChain = await fetchContractOnChainData(chainId, address);
        const parts = [];
        if (onChain.deployer) parts.push(`Criador: ${onChain.deployer}`);
        if (onChain.creationTx) parts.push(`Tx de criação: ${onChain.creationTx}`);
        if (onChain.recentTransfers) parts.push(`Transferências recentes: ${onChain.recentTransfers}`);
        if (onChain.lastActivity) parts.push(`Última atividade: ${onChain.lastActivity}`);
        if (parts.length) onChainSection = "\nDados on-chain:\n" + parts.join("\n");
      } catch (_) {}
    }

    const hasCode = !!(srcSnippet || abiSummary);
    const noCodeNote = !hasCode
      ? "\n\nINSTRUÇÃO OBRIGATÓRIA: Código-fonte e ABI não estão disponíveis. Descreva APENAS o que pode ser observado nos dados on-chain acima (criador, atividade). NÃO mencione risco, perigo, atenção ou qualquer avaliação de segurança. NÃO especule sobre capacidades ocultas."
      : "";

    const userPrompt = `Contrato: ${contractName || "Contrato sem nome"}${address ? `\nEndereço: ${address} (Chain ID: ${chainId})` : ""}${onChainSection}
Funções ABI: ${abiSummary || "(não disponível)"}
Código-fonte:
${srcSnippet || "(não disponível)"}${noCodeNote}`;
    ...
    let aiErrorMsg = "";
    if (groqKey) {
      const data = await callGroq(systemPrompt, userPrompt, groqKey);
      if (data?.error) {
        aiErrorMsg = `Groq: ${data.error.message || data.error.type || "erro"}`;
        console.warn("[analyze-contract] Groq error:", JSON.stringify(data.error));
      }
    ...
    if (!text && anthropicKey) {
      const data = await callAnthropic(systemPrompt, userPrompt, anthropicKey);
      if (data?.error) {
        aiErrorMsg += ` Anthropic: ${data.error.message || data.error.type || "erro"}`;
        console.warn("[analyze-contract] Anthropic error:", JSON.stringify(data.error));
      }
    ...
    if (!text) {
      return res.json({ success: false, reason: "api_error", error: aiErrorMsg || "Resposta vazia da IA" });
    }
    const analysis = text.trim();

    saveAnalysisToTraining({
      timestamp: new Date().toISOString(),
      contractName: contractName || "",
    ...
    return res.json({ success: true, analysis, model: modelUsed });
  } catch (e) {
    console.error("analyze-contract error:", e.message);
    return res.json({ success: false, error: e.message });
  }
});

// ── TokenCafe IA Assistant ─────────────────────────────────────────────────
const IA_SYSTEM_PROMPT = `Você é o TokenCafe Assistant, um especialista exclusivo em tokenização de ativos, blockchain tokens (principalmente ERC-20 e padrões relacionados), criação e gestão de tokens, Real World Assets (RWAs), smart contracts e todo o ecossistema técnico, regulatório e operacional relacionado a tokenização.

Seu conhecimento base é o site tokencafe.app: uma plataforma no-code que permite criar, validar e publicar tokens ERC-20 em mais de 30 blockchains de forma simples, sem cadastro obrigatório, sem mensalidade, pagando apenas na publicação. Você sempre pode mencionar as funcionalidades do TokenCafe quando relevante para ajudar o usuário com o sistema.

REGRAS RÍGIDAS DE COMPORTAMENTO (nunca as viole):

1. Escopo estrito: Responda APENAS sobre tokenização, seus componentes técnicos (standards como ERC-20/721/1155, minting, burning, supply, metadata, bridges, multi-chain deployment, validation, security audits, wallets, liquidity, etc.), RWAs, aspectos regulatórios e jurídicos de tokenização, comparação com plataformas concorrentes (Securitize, Tokeny, Polymath, Centrifuge, RealT, Lofty, Ondo Finance, tZERO, etc.), melhores práticas, riscos técnicos, processos de criação/gestão de tokens e informações diretamente relacionadas ao TokenCafe.app.

2. Recusa automática: Se a pergunta não tiver relação direta com tokenização, blockchain tokens, aspectos jurídicos/regulatórios de emissão e negociação de tokens, ou com o funcionamento do sistema TokenCafe, responda apenas: "Desculpe, eu sou especializado exclusivamente em tokenização e no ecossistema do TokenCafe.app. Não posso responder sobre esse tema."

3. Sem aconselhamento de investimentos: Nunca dê conselhos financeiros, recomendações de compra/venda, análise de rentabilidade, previsões de preço ou qualquer coisa que possa ser interpretada como aconselhamento de investimento. Use frases como "Isso não constitui aconselhamento financeiro. Consulte profissionais qualificados."

4. Aspectos jurídicos: Você pode trazer informações sobre leis e regulamentações de tokenização no Brasil (CVM, Banco Central, Lei 14.478/2022) e em outros países (SEC, Howey Test, MiCA, etc.). Sempre mencione que as informações são gerais e o usuário deve consultar advogados especializados.

5. Estilo: Seja claro, técnico quando necessário, objetivo e útil. Use exemplos práticos relacionados ao TokenCafe quando couber. Nunca saia do tema.

Responda sempre na língua em que o usuário escrever, mantendo tom profissional e especializado.`;
...
app.post("/api/ai-chat", async (req, res) => {
  ...
  if (!groqKey && !anthropicKey) {
    return res.json({ success: false, error: "Serviço de IA não configurado. Configure GROQ_API_KEY ou ANTHROPIC_API_KEY." });
  }

  const { messages } = req.body;
  if (!Array.isArray(messages) || messages.length === 0) {
    return res.json({ success: false, error: "Mensagens inválidas." });
  }
  ...
  if (validMessages.length === 0 || validMessages[validMessages.length - 1].role !== "user") {
    return res.json({ success: false, error: "Última mensagem deve ser do usuário." });
  }

  let reply = null;
  let modelUsed = null;

  // Tenta Groq primeiro (formato OpenAI — system como primeira mensagem)
  if (groqKey) {
    try {
      const groqMessages = [{ role: "system", content: IA_SYSTEM_PROMPT }, ...validMessages];
      const body = JSON.stringify({ model: "llama-3.3-70b-versatile", max_tokens: 1024, temperature: 0.7, messages: groqMessages });
        ...
        const r = https.request(opts, (response) => {
          ...
          response.on("end", () => {
            try {
              ...
              if (t) resolve(t); else reject(new Error(parsed?.error?.message || "Resposta inválida do Groq."));
            } catch (e) { reject(e); }
          });
        });
        r.on("error", reject); r.write(body); r.end();
      ...
      console.warn("ai-chat Groq fallback:", e.message);
    }
  }
  ...
  if (!reply && anthropicKey) {
    try {
      const body = JSON.stringify({ model: "claude-haiku-4-5-20251001", max_tokens: 1024, system: IA_SYSTEM_PROMPT, messages: validMessages });
      const text = await new Promise((resolve, reject) => {
        const opts = {
          hostname: "api.anthropic.com",
          path: "/v1/messages",
          method: "POST",
          headers: { "Content-Type": "application/json", "x-api-key": anthropicKey, "anthropic-version": "2023-06-01", "Content-Length": Buffer.byteLength(body) },
        };
        const r = https.request(opts, (response) => {
          let raw = "";
          response.on("data", c => raw += c);
          response.on("end", () => {
            try {
              const parsed = JSON.parse(raw);
              const t = parsed?.content?.[0]?.text;
              if (t) resolve(t); else reject(new Error(parsed?.error?.message || "Resposta inválida da IA."));
            } catch (e) { reject(e); }
          });
        });
        r.on("error", reject); r.write(body); r.end();
      ...
      console.warn("ai-chat Anthropic fallback:", e.message);
    }
  }

  if (!reply) {
    console.error("ai-chat: ambos os provedores falharam");
    return res.json({ success: false, error: "Não foi possível obter resposta da IA. Tente novamente em instantes." });
  }

  return res.json({ success: true, reply, model: modelUsed });
});
