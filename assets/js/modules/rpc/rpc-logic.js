...
async function initRPCManager() {
  ...
  // 1. Listeners PRIMEIRO, síncrono — garante que network:selected seja capturado
  //    antes de qualquer await (o auto-detect do componente roda em paralelo via fetch)
  setupEventListeners();

  // 2. RPCs externas em background — não bloqueia os listeners nem a UI
  loadExternalRpcs().catch(() => {});

  // 3. Verificar conexão existente sem forçar conexão
  const wcInit = getWalletConnector();
  if (wcInit && typeof wcInit.isConnected === "function") {
    const ok = await wcInit.isConnected();
    if (ok) {
      updateWalletUI(wcInit.getStatus());
    } else {
      // Apenas atualiza o header — NÃO chama hideAllSections() na inicialização.
      // resetWalletUI() ocultaria rpc-config-section se o auto-detect
      // já tivesse selecionado uma rede antes deste ponto.
  ...
  // 4. Garantir que a seção de busca de rede esteja visível
  showNextSection("network-section");

  // 5. Focar no campo de busca quando o componente estiver disponível
  try {
...
function setupEventListeners() {
  ...
  document.addEventListener("network:required", () => {
    hideNetworkDetails();
    clearNetworkForm({ silent: true });
    showNextSection("network-section");
  });
  document.addEventListener("network:clear", () => {
    clearNetworkForm({ clearSearch: false, silent: true });
  });
  ...
  registerModuleActions({ onClear: clearRpcSelection });
...
async function loadExternalRpcs() {
  ...
  let localEntries = [];
  try {
    const resLocal = await fetch("/data/rpcs.json");
    if (resLocal.ok) {
...
// Reseta apenas a seleção de RPC, preservando a rede selecionada.
// Isso evita que o usuário perca o contexto de rede ao tentar trocar o RPC.
function clearRpcSelection() {
  // Limpar RPC manual
  const customInput = document.getElementById("customRpcUrl");
  if (customInput) customInput.value = "";

  // Limpar opções de RPC e resetar estado de validação
  ...
  Object.keys(rpcValidationCache).forEach((k) => delete rpcValidationCache[k]);

  // Ocultar seção de adicionar e desabilitar botão
  ...
  // Manter rpc-config-section visível para o usuário escolher outro RPC
  document.getElementById("rpc-config-section")?.classList.remove("d-none");

  window.notify && window.notify("Seleção de RPC limpa. Escolha outro RPC para a rede.", "info");
}
...
function clearNetworkForm(options = {}) {
  ...
  if (clearSearch) {
    ...
    if (el) {
      ...
      // Não dispara evento "input" para evitar cascata: input → network:clear → clearNetworkForm recursivo
    }
    // Desabilitar botão de info diretamente
    const infoBtn = document.querySelector("#network-section #nsInfoBtn");
  ...
  if (addBtn) {
    addBtn.setAttribute("disabled", "");
    addBtn.classList.remove("d-none"); // garante que o botão reaparece na próxima seleção
  }

  // Garantir que a seção de busca permaneça visível e o usuário possa pesquisar novamente
  document.getElementById("network-section")?.classList.remove("d-none");
      ...
      (url, idx) => `
                        <div class="list-group-item d-flex align-items-center justify-content-between gap-2" data-rpc-url="${url}">
                            <div class="d-flex align-items-center gap-2">
                                ...
                                <button class="tc-icon-btn-ds tc-action-copy ms-1" type="button" onclick="window.copyToClipboard('${url}')" title="Copiar URL">
