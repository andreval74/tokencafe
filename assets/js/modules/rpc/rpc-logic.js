...
function clearNetworkForm(options = {}) {
  ...
  if (!silent) {
    // Only show this notification if it was an explicit user action to clear,
    // not during automatic UI updates or initializations
    const el = document.getElementById("networkSearch");
    if (el && !el.value) {
      // Don't show if the input is empty (initial state)
    } else {
