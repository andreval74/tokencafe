...
function clearAll() {
  // Preserva a rede atual para soft reset (retorno ao estado pós-seleção de rede sem recarregar)
  ...
  // Limpar campo e estado visual do contract-search
  const csInput = document.getElementById("f_address");
  if (csInput) csInput.value = "";
  try {
    const cs = document.querySelector('[data-component*="contract-search.php"]');
    if (typeof cs?.__tcContractSearchClear === "function") cs.__tcContractSearchClear({ silent: true });
  } catch (_) {}
  ...
  if (savedNetwork) {
    // Soft reset: restaura contexto da rede sem recarregar a página (sem piscada)
        ...
        <div class="modal-content">
          ...
          <div class="modal-body">
            <div class="d-grid gap-2">
              <button id="tcShareWhatsApp" class="tc-btn tc-btn-test-ds tc-action-whatsapp"><i class="bi bi-whatsapp me-2"></i>WhatsApp</button>
              <button id="tcShareTelegram" class="tc-btn tc-btn-test-ds tc-action-telegram"><i class="bi bi-telegram me-2"></i>Telegram</button>
              <button id="tcShareEmail" class="tc-btn tc-btn-test-ds tc-action-email"><i class="bi bi-envelope me-2"></i>Email</button>
              <button id="tcShareCopy" class="tc-btn tc-btn-test-ds tc-action-copy"><i class="bi bi-clipboard me-2"></i>Copiar Link</button>
            </div>
          </div>
        </div>
      </div>`;
