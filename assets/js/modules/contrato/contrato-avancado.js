...
class AdvancedTokenPageManager {
        ...
        navBtns.forEach(btn => {
            btn.addEventListener('click', (e) => {
                ...
                if (targetTabBtn) {
                    ...
                    setTimeout(() => {
                        document.getElementById('advancedGeneratorForm')?.scrollIntoView({ behavior: 'smooth' });
                    }, 100);
                } else {
     ...
     * O formulário avançado usa layout de página única (sem abas), portanto
     * o resumo é recalculado nos eventos de input e ao concluir o deploy.
     */
    bindSummaryUpdates() {
            ...
            features.forEach(feat => {
                ...
                col.innerHTML = `
                    <div class="d-flex align-items-center p-2 rounded border border-secondary bg-dark-elevated">
                        ...
                        <span class="text-white tc-text-sm fw-bold">${feat.label}</span>
                    </div>
                `;
    ...
    bindDeploy() {
        ...
        // Botão Limpar / Reset (aceita ambos os IDs por compatibilidade)
        const btnReset = document.getElementById("btnResetForm") || document.getElementById("btnClearAdvanced");
        if (btnReset) {
