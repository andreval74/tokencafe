...
import { getExplorerContractUrl, getExplorerTxUrl, getExplorerVerificationUrl } from "../../shared/explorer-utils.js";
...
export const state = {
  ...
  deployed: {
    ...
    txReferrer: null,
    feeSymbol: null,
    feePlatformAmount: null,
    feeReferrerAmount: null,
    feePlatformUSD: null,
    feeTotalUSD: null,
    billingAddress: null,   // carteira que pagou (origem de todas as TXs)
    referrerAddress: null,  // carteira do indicador (destino do bônus)
    platformWallet: null,   // carteira da plataforma (destino da taxa)
    gasEstimateCrypto: null, // custo estimado de gas (deploy)
  },
};
...
/**
 * Restaura state.form a partir de um objeto salvo no sessionStorage.
 * Usado pela página contrato-deploy para retomar sem precisar do formulário DOM.
 */
export function hydrateStateFromPending(config) {
  if (!config) return;
  if (config.network)                     state.form.network       = config.network;
  if (config.group)                       state.form.group         = config.group;
  if (config.token)                       state.form.token         = { ...config.token };
  if (config.sale) {
    state.form.sale = { ...config.sale };
    // capUnits serialized as string (BigInt não sobrevive JSON); restaura
    try { state.form.sale.capUnits = BigInt(config.sale.capUnits ?? "0"); } catch (_) { state.form.sale.capUnits = 0n; }
  }
  if (config.initialOwner  !== undefined) state.form.initialOwner  = config.initialOwner;
  if (config.initialHolder !== undefined) state.form.initialHolder = config.initialHolder;
  if (config.advanced)                    state.form.advanced      = JSON.parse(JSON.stringify(config.advanced));
}
  ...
  box.innerHTML = `
    <div class="alert alert-dark border p-3">
      <div class="d-flex align-items-center mb-1">
        <strong class="tc-text-sm">${info.title}</strong>
      </div>
      ${saleBadge}
      <div class="tc-text-sm mb-2">${info.summary}</div>
      <div class="tc-text-sm text-muted">Ordem: ${info.order.join(" → ")}</div>
      <div class="mt-2 tc-text-sm">Funções: ${info.features.join(", ")}</div>
      <div class="mt-2 tc-text-sm">Notas: ${info.notes}</div>
    </div>
  `;
}
...
export async function connectWallet() {
  try {
    ...
    // walletConnector é a fonte de verdade — conta mostrada no header
    // getSigner() sem arg pegaria a conta ativa do MetaMask que pode diferir
    const wcAccount = window.walletConnector?.getStatus?.()?.account || null;
    const signer = wcAccount ? provider.getSigner(wcAccount) : provider.getSigner();
    const address = wcAccount || await signer.getAddress();
...
function showConnectionDiagnosis(attempts) {
  const list = attempts.map(a => `<li class="mb-2 p-2 border rounded bg-white"><div class="d-flex justify-content-between align-items-center"><span class="badge bg-secondary">${a.label}</span></div><span class="text-muted d-block mt-1 text-break tc-text-sm">${a.url}</span><span class="text-danger d-block mt-1 tc-text-sm"><i class="bi bi-x-circle"></i> ${a.status}</span></li>`).join("");
  ...
  const content = `
        <div class="text-start">
            ...
            <ul class="tc-text-sm text-muted mb-3">
  ...
  try {
    ...
    if (help) {
      help.innerHTML = `<strong>Falha Geral:</strong> <br> <span class="tc-text-sm">Nenhum servidor respondeu. Veja o popup para detalhes.</span>`;
...
export async function ensureServersOnline() {
  ...
  // 1. Tentar base configurada (api-config.js já escolhe localhost em dev)
  if (await tryConnect(base, "Base Configurada")) {
    await checkApiEndpoints(base);
    return true;
  }

  // 2. Em dev local, tentar localhost:3000 explicitamente antes de ir para produção
  const local3000 = "http://localhost:3000";
  if (base !== local3000) {
    log("Tentando API local (localhost:3000)...");
    if (await tryConnect(local3000, "Local Dev")) {
      setApiBase(local3000);
      await checkApiEndpoints(local3000);
      log("Conectado à API local (localhost:3000).");
      return true;
    }
  }
  ...
  try {
    ...
    if (err instanceof TypeError && msg.includes("Failed to fetch")) {
      ...
      if (pageProto === "https:" && apiProto === "http:") {
        log(`Bloqueio Mixed Content: página HTTPS não pode acessar API HTTP. Verifique a URL da API.`);
      } else {
        log(`Falha de rede (timeout/conexão). Página: ${pageProto}, URL: ${apiProto}, online=${navigator.onLine}`);
      }
    }
    throw err;
  }
...
export async function compileContract(opts = {}) {
  ...
  try {
    if (!opts.skipUIValidation) readForm();
        ...
        if (window.showVerificationResultModal) {
          window.showVerificationResultModal(
            ...
            `<div class="text-center">
               ...
               <p class="text-muted tc-text-sm">Endereço: ${state.deployed.address}</p>
             </div>`,
            explorerUrl,
          );
        }
    ...
    try {
      ...
      if (group === "erc20-minimal") {
        ...
        try {
          ...
          // skipFeePayment=true quando a taxa já foi paga antes de chamar deployContract
          const feeResult = opts.skipFeePayment
              ? { ok: true, signer }
              : await feeMgr.confirmAndPay(signer, state.form.network, gasLimit, state.form.group);
          ...
          // Enviar transação — gasPrice explícito evita falha de EIP-1559 em BSC
          log("Enviando transação manual...");
          let gasPrice;
          try { gasPrice = await signer.getGasPrice(); } catch (_) {}
        ...
        const feeResult = opts.skipFeePayment
            ? { ok: true, signer }
            : await feeMgr.confirmAndPay(signer, state.form.network, gasLimitBN, state.form.group);
        const feeOk = !!(feeResult && (feeResult.ok ?? feeResult === true));
        if (!feeOk) {
          log("Deploy cancelado pelo usuário ou falha no pagamento da taxa.");
          stopOpStatus("Cancelado");
        ...
        // gasPrice explícito evita falha de EIP-1559 em redes que usam gas legado (BSC)
        try { const gp = await signer.getGasPrice(); if (gp) overrides.gasPrice = gp; } catch (_) {}
      ...
      // Verificação no Explorer: admin pode sempre; não-admin apenas em mainnet.
      let _skipVerify = false;
      try {
        const _cid = state.deployed?.chainId || state.form?.network?.chainId;
        if (_cid && nm.isTestNetwork(_cid) && !checkIsAdmin()) _skipVerify = true;
      } catch (_) {}
      ...
      if (!_skipVerify) {
        ...
        log("Verificação no Explorer ignorada (testnet ou sem permissão admin).");
      }
...
async function initWalletIfConnected() {
  try {
    ...
    const provider = new ethers.providers.Web3Provider(window.ethereum);
    // WalletConnector é a fonte de verdade — mesmo padrão de connectWallet()
    const wcAccount = window.walletConnector?.getStatus?.()?.account || null;
    const signer = wcAccount ? provider.getSigner(wcAccount) : provider.getSigner();
    const address = wcAccount || await signer.getAddress();
    const { chainId } = await provider.getNetwork();
...
async function bindUI() {
  ...
  // owner/holder só preenchido ao conectar: evita usar endereço stale do localStorage
  try {
      ...
      try {
        ...
        try {
          ...
          try {
            ...
            // REDIRECT REMOVIDO: Fluxo agora é Single Page App
          } catch (e) {
      ...
      if (kind === "bsc") {
        ...
        html = `
          ...
          <div class="alert alert-dark">
            ...
            <div class="mt-2 tc-text-sm">Compiler/Optimizer conforme compilação atual</div>
          </div>
        ...
        html = `
          ...
          <p class="tc-text-sm">Guarde seus arquivos (.sol/.json/.hex) na seção de downloads para auditorias futuras.</p>
        `;
    ...
    if (launch) {
      launch.addEventListener("click", async () => {
        ...
        try {
          ...
          if (tx && !launch.disabled) tx.textContent = "Analisar Contrato";
        } catch (_) { }
      });
    }
  } catch (_) { }
...
export async function verifyCurrentContract() {
  // Verificar no explorer: admin pode sempre; não-admin só em mainnet.
  // A checagem é FORA do try/catch para evitar que exceções silenciosas
  // permitam verificação indevida em testnet.
  let isTestnet = false;
  try {
    const cid = buildVerifyPayloadFromState()?.chainId;
    if (cid) isTestnet = nm.isTestNetwork(cid);
  } catch (_) {}
