...
function openContractDetailsCard() {
  try {
    const csContainer = document.querySelector('[data-component="modules/contrato/contract-search.php"]');
...
(function () {
  ...
  registerModuleActions({ onClear: doClearAll });
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  const container = document.querySelector('[data-component="modules/contrato/contract-search.php"]');
  if (container) {
    let attempts = 0;
  ...
  // Botões de ação do código-fonte
  const importSourceBtn = document.getElementById("importSourceBtn");
  const importSourceFile = document.getElementById("importSourceFile");
  ...
  const fSourceCode = document.getElementById("f_sourceCode");

  if (importSourceBtn && importSourceFile) {
    ...
    importSourceFile.addEventListener("change", (e) => {
      ...
      reader.onload = (ev) => {
        if (fSourceCode) {
          fSourceCode.value = ev.target.result;
          fSourceCode.dispatchEvent(new Event("input"));
        }
      };
  ...
  if (pasteSourceBtn && fSourceCode) {
    pasteSourceBtn.addEventListener("click", async () => {
      try {
        const text = await navigator.clipboard.readText();
        fSourceCode.value = text;
        fSourceCode.dispatchEvent(new Event("input"));
      } catch (_) {
        window.notify?.("Não foi possível acessar a área de transferência", "error");
      }
    });
  }

  if (copySourceBtn && fSourceCode) {
    copySourceBtn.addEventListener("click", () => {
      if (fSourceCode.value) {
        navigator.clipboard.writeText(fSourceCode.value).catch(() => {});
        window.notify?.("Código copiado!", "success");
      }
    });
  }

  if (clearSourceBtn && fSourceCode) {
  ...
  try {
    ...
    if (r.success) {
      ...
      const errHtml = `
        ...
        <div class="alert alert-secondary text-light tc-text-sm">
