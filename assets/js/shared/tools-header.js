/**
 * tools-header.js — Inicializador da navbar das ferramentas (app shell)
 * Lê data-attributes do elemento [data-component="tools-header.php"] e
 * atualiza #tools-header-title/#tools-header-subtitle com o título do módulo.
 * Gerencia também o chip de carteira, botão de logout e sidebar mobile.
 */
  ...
  try {
    ...
    containers.forEach((el) => {
      ...
      const renderWalletMenu = async (selected) => {
        ...
        if (!multi) {
          walletChipMenu.innerHTML = `
              <li><span class="dropdown-item-text text-white-50 tc-text-sm">Apenas 1 conta autorizada nesta conexão.</span></li>
              <li>
      ...
      window.handleLogout =
        ...
        (async function () {
          ...
          try { localStorage.removeItem("tokencafe_wallet_cache");              } catch {}
          try { localStorage.removeItem("tokencafe_wallet_address");            } catch {}
          try { localStorage.removeItem("tokencafe_wallet_session_authorized"); } catch {}
          try { sessionStorage.removeItem("tokencafe_wallet_cache");             } catch {}
          try { sessionStorage.removeItem("tokencafe_wallet_session_authorized");} catch {}
          try { sessionStorage.removeItem("tokencafe_wallet_address");           } catch {}
          try { document.cookie = "tokencafe_wallet_address=; Path=/; Max-Age=0; SameSite=Lax";              } catch {}
          try { document.cookie = "tokencafe_wallet_session_authorized=; Path=/; Max-Age=0; SameSite=Lax";  } catch {}
          try {
