...
export class WalletConnector {
  constructor() {
    ...
    // Cache e performance
    this.cache = new Map();
    this.cacheTimeout = 7 * 24 * 60 * 60 * 1000; // 7 dias
    ...
    // Selo de sessão: usa sessionStorage — expira quando o browser é fechado.
    // O usuário precisa reconectar a carteira a cada nova sessão do browser.
    try {
      this.sessionAuthorized = sessionStorage.getItem("tokencafe_wallet_session_authorized") === "true";
      localStorage.removeItem("tokencafe_wallet_session_authorized"); // limpa flag legada
    } catch (_) {
  ...
  async connect(walletType = "metamask") {
    try {
      ...
      // Marcar sessão como autorizada (ação explícita do usuário).
      // sessionStorage: some quando o browser é fechado → obriga novo login.
      this.sessionAuthorized = true;
      try {
        sessionStorage.setItem("tokencafe_wallet_session_authorized", "true");
      } catch (_) {}
      try {
        // Cookie de sessão sem Max-Age → expira ao fechar o browser (nunca persiste em disco).
  ...
  async disconnect() {
    try {
      ...
      // Limpar cache e selo de sessão (clearCache já limpa sessionStorage e localStorage)
      this.clearCache();
      this.sessionAuthorized = false;
  ...
  isWalletAvailable(walletType) {
    ...
    switch (walletType) {
      ...
      case "trust":
        return !!eth && (eth.isTrust === true || eth.isTrustWallet === true);
      case "coinbase":
        return !!eth && (eth.isCoinbaseWallet === true);
      // Fallback genérico: qualquer provider EIP-1193 injetado (ex: outros wallets mobile)
      case "ethereum":
  ...
  saveConnectionCache() {
    ...
    sessionStorage.setItem("tokencafe_wallet_cache", JSON.stringify(cacheData));
  }
  ...
  getConnectionCache() {
    try {
      const cached = sessionStorage.getItem("tokencafe_wallet_cache");
   ...
   */
  clearCache() {
    sessionStorage.removeItem("tokencafe_wallet_cache");
    sessionStorage.removeItem("tokencafe_wallet_session_authorized");
    localStorage.removeItem("tokencafe_wallet_session_authorized"); // garante limpeza de legado
    this.cache.clear();
    this.isConnected = false;
    this.currentAccount = null;
    this.connectedWallet = null;
