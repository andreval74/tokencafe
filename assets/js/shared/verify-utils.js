...
// Chave lida de api/.env e injetada pelo PHP via head.php
const FALLBACK_EXPLORER_API_KEY = window.TOKENCAFE_EXPLORER_API_KEY || "";
...
async function checkExplorerDirectly(chainId, address) {
    ...
    if (baseUrl.includes("etherscan.io/v2/")) {
        ...
        // getsourcecode funciona sem key no tier gratuito (rate limit mais baixo, OK para polling)
        const key = getVerifyApiKey() || FALLBACK_EXPLORER_API_KEY;
        if (key) {
            url += `&apikey=${key}`;
        }
        // sem key: tenta sem parâmetro (BSCScan/PolygonScan aceitam com limite de taxa)
    }
    ...
    try {
        ...
        try {
            ...
            if (guid) {
                ...
                // Quando GUID está pending, NÃO retornamos cedo: o BSCScan pode já ter processado
                // a verificação antes de atualizar o status do GUID. Continuamos para checar
                // via backend/direto, usando o pending apenas como fallback final.
            }
        } catch (_) {}
