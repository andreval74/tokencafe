 ...
 * 
 * O componente é carregado via data-component="includes/api-status.php".
...
export async function checkConnectivity(showOnCheck = false, overrideBaseUrl = null) {
    ...
    if (badge) {
        badge.className = "tc-badge-module tc-status-warn";
    ...
    try {
        ...
        if (resp.ok) {
            ...
            if (badge) {
                badge.className = "tc-badge-module tc-status-ok";
            ...
            if (badge) {
                badge.className = "tc-badge-module tc-status-bad";
...
function findContainers() {
    // Procura por containers que carregaram o componente api-status.php
    ...
    const nodes = document.querySelectorAll('[data-component*="api-status.php"]');
