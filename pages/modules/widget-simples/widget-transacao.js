...
(function(){
  ...
  const log = (obj) => { const el = $('#tw-log'); if (el) el.textContent = JSON.stringify(obj, null, 2); };
  ...
  class TransactionWidget {
    constructor(network, contractAddress) {
      ...
      this.state = { contractIsDeployed: null, isErc20Valid: null, canTransfer: null, sellerBalance: null, allowanceOk: null };
    ...
    async init() {
      ...
      this.readProv = picked.prov; this.readRpcUrl = picked.url;
      try {
    ...
    attachEvents() {
      this.el.contractInput?.addEventListener('change', () => { this.contractAddress = this.el.contractInput.value.trim(); });
      this.el.tokenInput?.addEventListener('change', () => { this.tokenAddress = this.el.tokenInput.value.trim(); });
      this.el.priceInput?.addEventListener('change', () => {
        const v = this.el.priceInput.value.trim();
        try { this.pricePerToken = ethers.utils.parseEther(v || '0.001'); } catch(_) { this.pricePerToken = DEFAULT_PRICE_PER_TOKEN; }
        this.refreshTotal();
      });
      ...
      this.el.connectBtn?.addEventListener('click', async () => {
        try {
          ...
          if (this.el.buyerInfo) { this.el.buyerInfo.innerHTML = `<span class="text-success"><i class="bi bi-person-bounding-box me-2"></i>${this.buyerWallet}</span>`; }
    ...
    async refreshBalances() {
      try {
        ...
        log({ warn: 'Falha ao atualizar saldos', details: e?.message || String(e) });
      }
    }
    ...
    async updateNetworkInfo() {
      try {
        ...
        log({ warn: 'Falha ao atualizar rede atual', details: e?.message || String(e) });
      }
      try {
    ...
    async buy() {
      try {
        if (!signer) { log({ error: 'Conecte a carteira' }); return; }
        await ensureWalletNetwork();
        if (!this.contractAddress || !ethers.utils.isAddress(this.contractAddress)) { log({ error: 'Informe contrato válido' }); return; }
        const qty = Number(this.el.qtyInput?.value || 1);
        if (qty <= 0) { log({ error: 'Quantidade inválida' }); return; }
        const total = this.pricePerToken.mul(qty);
        const buyerBal = await signer.getBalance();
        if (buyerBal.lt(total)) { log({ error: 'Saldo tBNB insuficiente', balance: buyerBal.toString(), total: total.toString() }); return; }

        const buyBtn = this.el.buyBtn; if (buyBtn) { buyBtn.disabled = true; buyBtn.innerHTML = '<span class="spinner-border spinner-border-sm me-2"></span>Processando...'; }
        ...
        try {
          await sale.callStatic.buy(ethers.BigNumber.from(qty), { value: total });
        } catch (e) {
          ...
          if (buyBtn) { buyBtn.disabled = false; buyBtn.innerHTML = '<i class="bi bi-cash-coin me-2"></i> Confirmar compra'; }
          return;
        }
        try {
          ...
          tx = await sale.buy(ethers.BigNumber.from(qty), { value: total, gasLimit });
        } catch (_) {
          tx = await signer.sendTransaction({ to: this.contractAddress, value: total });
        }
        ...
        const buyBtn = this.el.buyBtn; if (buyBtn) { buyBtn.disabled = false; buyBtn.innerHTML = '<i class="bi bi-cash-coin me-2"></i> Confirmar compra'; }
    ...
    async addToMetaMask() {
      try {
        if (!window.ethereum) { log({ error: 'MetaMask não disponível' }); return; }
        if (!this.tokenAddress || !ethers.utils.isAddress(this.tokenAddress)) { log({ error: 'Token inválido' }); return; }
        const erc20 = new ethers.Contract(this.tokenAddress, ERC20_ABI, this.readProv);
        const [symbol, decimals] = await Promise.all([
        ...
        const res = await window.ethereum.request({
          ...
          params: {
            type: 'ERC20',
            options: { address: this.tokenAddress, symbol: symbol.slice(0, 12), decimals: Number(decimals) }
          }
        });
        log({ action: 'wallet_watchAsset', result: res });
      } catch(e) {
        log({ error: 'Falha ao adicionar ao MetaMask', details: e?.message || String(e) });
      }
    }
  }
