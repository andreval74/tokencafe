...
(function(){
  ...
  const logEl = $('#output');
  const log = (obj) => { if (logEl) logEl.textContent = JSON.stringify(obj, null, 2); };
  ...
  const TOKEN_SALE_ABI = [
    ...
    "function purchasedBy(address) view returns (uint256)",
  ...
  function withTimeout(promise, ms, label = 'timeout') {
    return new Promise((resolve) => {
      ...
      Promise.resolve(promise)
        .then((val) => { settled = true; clearTimeout(timer); resolve(val); })
        .catch((err) => { settled = true; clearTimeout(timer); resolve(Promise.reject(err)); });
    });
  }
  ...
  async function pickResponsiveRpc(timeoutMs = 8000) {
    ...
    readProviderCache = prov; readRpcUrlCache = fallbackUrl;
  ...
  async function connectWallet() {
    try {
      ...
      log({ error: 'Falha ao conectar carteira', details: e?.message || String(e) });
    }
  }
  ...
  async function readSale() {
    try {
      ...
      try {
        ['saleToken','tokenDecimals','destWallet','bnbPrice','perWalletCap','contractTokenBalance','alreadyPurchased','totalCost']
          .forEach(id => { const el = document.getElementById(id); if (el) el.textContent = '...'; });
      } catch(_) {}

      const hasCode = await isContract(saleAddr, readProv);
      if (!hasCode) { log({ error: 'Endereço não tem contrato na rede 97', address: saleAddr }); return; }
      ...
      const results = await Promise.allSettled([
        withTimeout(sale.saleToken(), 8000, 'saleToken'),
        withTimeout(sale.destinationWallet(), 8000, 'destinationWallet'),
        withTimeout(sale.bnbPrice(), 8000, 'bnbPrice'),
        withTimeout(sale.perWalletCap(), 8000, 'perWalletCap')
      ]);
      ...
      } catch (_) { const el = document.getElementById('saleBnbBalance'); if (el) el.textContent = '-'; }
      ...
      try {
        ...
        if (destWallet && ethers.utils.isAddress(destWallet)) {
          ...
          const el = document.getElementById('destBnbBalance'); if (el) el.textContent = '-';
        }
      } catch (_) { const el = document.getElementById('destBnbBalance'); if (el) el.textContent = '-'; }
      ...
      try {
        ...
        if (buyer) {
          ...
          const el = document.getElementById('buyerBnbBalance'); if (el) el.textContent = '-';
        }
      } catch (_) { const el = document.getElementById('buyerBnbBalance'); if (el) el.textContent = '-'; }
      ...
      try {
        ...
        if (buyer && saleTokenAddr && saleTokenAddr !== ethers.constants.AddressZero) {
          ...
          const el = document.getElementById('buyerTokenBalance'); if (el) el.textContent = '-';
        }
      } catch (_) { const el = document.getElementById('buyerTokenBalance'); if (el) el.textContent = '-'; }
      ...
      log({ ok: true, action: 'readSale-done', saleAddr, saleToken: saleTokenAddr, destinationWallet: destWallet,
            bnbPrice: bnbPrice?.toString() || null, perWalletCap: perWalletCap?.toString() || null,
            tokenDecimals: String(tokenDecimals), contractTokenBalance, warnings, debug });
    } catch (e) {
      log({ error: 'Falha ao ler dados do contrato', details: e?.message || String(e), where: 'readSale', rpcUrl: getSelectedRpcUrl() });
    }
  }
  ...
  async function testDestinationWallet(destWallet) {
    try {
      ...
      const gas = await readProv.estimateGas({ to: destWallet, from, value: ethers.BigNumber.from(1) });
  ...
  async function certifySale(qty) {
    try {
      ...
      if (!(await isContract(saleAddr, readProv))) { log({ error: 'Endereço não tem contrato na rede 97', address: saleAddr }); return; }
      ...
      const res = await Promise.allSettled([
        ...
        withTimeout(sale.bnbPrice(), 8000, 'bnbPrice'),
        withTimeout(sale.perWalletCap(), 8000, 'perWalletCap'),
        withTimeout(sale.purchasedBy(buyer), 8000, 'purchasedBy')
      ]);
      ...
      // saldo e decimals
      let tokenBalance = null, tokenDecimals = null;
      if (saleTokenAddr) {
        try {
          const erc20 = new ethers.Contract(saleTokenAddr, ERC20_ABI, readProv);
      ...
      log({ compatible: problems.length === 0, action: 'certifySale-done', problems, warnings, checks, debug });
    } catch (e) {
      log({ error: 'Falha na certificação', details: e?.message || String(e), where: 'certifySale', rpcUrl: getSelectedRpcUrl() });
    }
  }
  ...
  async function buyWithBNB() {
    try {
      await ensureNetworkWallet();
      const saleAddr = $('#saleAddress').value.trim();
      if (!saleAddr) { log({ error: 'Informe o endereço do contrato TokenSale' }); return; }
      const qty = Number($('#qty').value || 1);
      if (qty <= 0) { log({ error: 'Quantidade inválida' }); return; }
      const buyBtn = $('#buyBtn');
      if (buyBtn) { buyBtn.disabled = true; buyBtn.innerHTML = '<span class="spinner-border spinner-border-sm me-2"></span> Comprando...'; }

      const sale = new ethers.Contract(saleAddr, TOKEN_SALE_ABI, signer);
      let bnbPrice = null;
      try { bnbPrice = await withTimeout(sale.bnbPrice(), 8000, 'bnbPrice'); } catch (_) {}
      if (!bnbPrice) { log({ error: 'bnbPrice indisponível — confirme se o endereço é do TokenSale (não Proxy)' }); if (buyBtn){ buyBtn.disabled=false; buyBtn.innerHTML='<i class="bi bi-cash-coin me-2"></i> Comprar com tBNB'; } return; }

      // Detectar unidades: tentar qty em inteiro e em unidades (decimals)
      let saleTokenAddr = null, tokenDecimals = 18, qtyUnits = null;
      try {
      ...
      try {
        ...
        if (qtyUnits && totalB) {
          try {
            ...
            chosenQty = qtyUnits; chosenTotal = totalB;
          } catch (eB) {
            log({ error: 'Falha na compra (static)', tryA: eA?.message || String(eA), tryB: eB?.message || String(eB), debug: { qty, qtyUnits: qtyUnits?.toString(), totalA: totalA.toString(), totalB: totalB?.toString(), decimals: tokenDecimals } });
            if (buyBtn){ buyBtn.disabled=false; buyBtn.innerHTML='<i class="bi bi-cash-coin me-2"></i> Comprar com tBNB'; }
            return;
          }
        } else {
          log({ error: 'Falha na compra (static)', details: eA?.message || String(eA), debug: { qty, totalA: totalA.toString() } });
          if (buyBtn){ buyBtn.disabled=false; buyBtn.innerHTML='<i class="bi bi-cash-coin me-2"></i> Comprar com tBNB'; }
          return;
        }
      }
      ...
      try {
        ...
        if (balance.lt(chosenTotal)) {
          log({ error: 'Saldo tBNB insuficiente', balance: balance.toString(), totalCost: chosenTotal.toString(), scenario: (chosenQty.eq(qtyUnits) ? 'units' : 'integer') });
          if (buyBtn){ buyBtn.disabled=false; buyBtn.innerHTML='<i class="bi bi-cash-coin me-2"></i> Comprar com tBNB'; }
          return;
        }
      } catch (_) { $('#totalCost').textContent = chosenTotal.toString(); }
      ...
      try {
        ...
        if (bal.lt(chosenQty)) {
          log({ error: 'Saldo de tokens insuficiente no contrato', contractTokenBalance: bal.toString(), requested: chosenQty.toString() });
          if (buyBtn){ buyBtn.disabled=false; buyBtn.innerHTML='<i class="bi bi-cash-coin me-2"></i> Comprar com tBNB'; }
          return;
        }
        if (cap && !cap.isZero()) {
          ...
          if (chosenQty.gt(remaining)) {
            log({ error: 'Limite por carteira excedido', cap: cap.toString(), already: already.toString(), remaining: remaining.toString(), requested: chosenQty.toString() });
            if (buyBtn){ buyBtn.disabled=false; buyBtn.innerHTML='<i class="bi bi-cash-coin me-2"></i> Comprar com tBNB'; }
            return;
          }
        }
      ...
      try {
        ...
        log({ warn: 'estimateGas falhou; usando gasLimit=250000', details: e?.message || String(e), link: 'https://links.ethers.org/v5-errors-UNPREDICTABLE_GAS_LIMIT' });
      }
      ...
      if (receipt.status !== 1) {
        ...
        try {
          ...
          const { prov: readProv } = await pickResponsiveRpc(5000);
          let buyerTokenBal = null, contractTokenBal = null;
          if (saleTokenAddr && saleTokenAddr !== ethers.constants.AddressZero) {
            const erc20 = new ethers.Contract(saleTokenAddr, ERC20_ABI, readProv);
            ...
            const elBuyer = document.getElementById('buyerTokenBalance'); if (elBuyer) elBuyer.textContent = buyerTokenBal.toString();
            const elContract = document.getElementById('contractTokenBalance'); if (elContract) elContract.textContent = contractTokenBal.toString();
          }
          // atualizar já comprados
          try { const purchased = await withTimeout(sale.purchasedBy(buyer), 8000, 'purchasedBy(post)'); const el = document.getElementById('alreadyPurchased'); if (el) el.textContent = purchased.toString(); } catch(_){ }
          // resumo
          const resumo = {
            ok: true,
            action: 'buy-done',
          ...
          log({ ok: true, action: 'buyWithBNB', txHash: tx.hash, receipt, debug: { qty, qtyUnits: qtyUnits?.toString(), chosenQty: chosenQty.toString(), chosenTotal: chosenTotal.toString() } });
        }
      }
      if (buyBtn){ buyBtn.disabled=false; buyBtn.innerHTML='<i class="bi bi-cash-coin me-2"></i> Comprar com tBNB'; }
    } catch (e) {
      log({ error: 'Falha na compra com BNB', details: e?.message || String(e) });
      const buyBtn = $('#buyBtn');
      if (buyBtn){ buyBtn.disabled=false; buyBtn.innerHTML='<i class="bi bi-cash-coin me-2"></i> Comprar com tBNB'; }
    }
  }
  ...
  function bindUI() {
    ...
    readBtn?.addEventListener('click', async () => {
      ...
      try {
        if (readBtn) { readBtn.disabled = true; readBtn.classList.add('tc-btn-disabled'); readBtn.innerHTML = '<span class="spinner-border spinner-border-sm me-2"></span>Lendo...'; }
        await readSale();
      } finally {
        if (readBtn) { readBtn.disabled = false; readBtn.classList.remove('tc-btn-disabled'); readBtn.innerHTML = '<i class="bi bi-search me-2"></i>Ler dados do contrato'; }
      }
    });
    certifyBtn?.addEventListener('click', async () => {
      ...
      try {
        if (certifyBtn) { certifyBtn.disabled = true; certifyBtn.classList.add('tc-btn-disabled'); certifyBtn.innerHTML = '<span class="spinner-border spinner-border-sm me-2"></span>Certificando...'; }
        await certifySale(qty);
      } finally {
        if (certifyBtn) { certifyBtn.disabled = false; certifyBtn.classList.remove('tc-btn-disabled'); certifyBtn.innerHTML = '<i class="bi bi-shield-check me-2"></i>Certificar contrato (tBNB)'; }
      }
    });
  ...
  function init() {
    ...
    try {
      window.__selectedNetwork = { chainId: REQUIRED_CHAIN_ID, nativeCurrency: { symbol: 'tBNB' } };
