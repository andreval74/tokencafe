...
if ($fltHour !== "") {
  if (!preg_match('/^\d{1,2}$/', $fltHour)) {
    http_response_code(400);
    echo "Hora inválida.";
    exit;
  }
  $fltHour = str_pad($fltHour, 2, "0", STR_PAD_LEFT);
}
if (!in_array($type, ["visits", "client", "ip", "sc", "report"], true)) {
  http_response_code(400);
  echo "Tipo inválido.";
  exit;
}
...
if (!$isChief) {
  ...
  if (!in_array($type, ["ip", "sc", "report"], true)) {
    http_response_code(403);
    echo "Acesso negado.";
    exit;
  }
...
function files_for_date(string $type, string $date): array {
  if ($type === "report") {
    return [
      __DIR__ . "/modules/relatorios/storage/admin-logs/IPLogs-" . $date . ".php",
      __DIR__ . "/modules/relatorios/storage/admin-logs/SCLogs-" . $date . ".php",
    ];
  }
  if ($type === "ip" || $type === "sc") {
    $base = $type === "ip" ? "IPLogs" : "SCLogs";
    return [__DIR__ . "/modules/relatorios/storage/admin-logs/" . $base . "-" . $date . ".php"];
  }
  return [__DIR__ . "/modules/relatorios/storage/logs/" . $type . "-" . $date . ".log"];
}
...
if ($rangeMode) {
  ...
  for ($ts = $tsStart; $ts <= $tsEnd; $ts += 86400) {
    $d = gmdate("Y-m-d", $ts);
    foreach (files_for_date($type, $d) as $p) {
  ...
  $paths = files_for_date($type, $date);
  foreach ($paths as $p) {
    if (is_file($p)) $files[] = $p;
  }
  if (!$files) {
...
function dl_txt_range(array $paths, string $filename, bool $skipCsvHeaderOnNext = true): void {
  ...
  foreach ($paths as $path) {
    ...
    while (!feof($fh)) {
      ...
      if (!$firstFile && $skipCsvHeaderOnNext && ($t === "data;hora;ip;wallet;page" || $t === "data;hora;ip;wallet;page;chain;contract" || $t === "data;hora;ip;wallet;page;chain;contract;action")) continue;
      echo $line;
    }
    fclose($fh);
    $firstFile = false;
...
function match_filters(array $row, string $page, string $ip, string $wallet, string $contract, string $hour): bool {
  ...
  if ($contract !== "") {
    $rc = strtolower((string) ($row["contract"] ?? ""));
    if ($rc === "" || str_contains($rc, $contract) === false) return false;
  }
  if ($hour !== "") {
...
function dl_txt_filtered(string $path, string $page, string $ip, string $wallet, string $contract, string $hour): void {
  ...
  header("Content-Disposition: attachment; filename=\"" . preg_replace('/\.(php|log)$/', '.txt', basename($path)) . "\"");
  header("X-Content-Type-Options: nosniff");
  echo "data;hora;ip;wallet;page;chain;contract;action\n";
  ...
  foreach ($lines as $ln) {
    ...
    echo ($r["date"] ?? "") . ";" . ($r["time"] ?? "") . ";" . ($r["ip"] ?? "") . ";" . ($r["wallet"] ?? "") . ";" . ($r["page"] ?? "") . ";" . ($r["chain"] ?? "") . ";" . ($r["contract"] ?? "") . ";" . ($r["action"] ?? "") . "\n";
  }
  exit;
}
...
function dl_txt_report_range(array $paths, string $filename, string $type, string $start, string $end, string $page, string $ip, string $wallet, string $contract, string $hour, bool $includeIp): void {
  ...
  header("Content-Disposition: attachment; filename=\"" . $filename . "\"");
  header("X-Content-Type-Options: nosniff");
  $tLabel = $type === "report" ? "Relatório (IPLogs + SCLogs)" : ($type === "ip" ? "IPLogs (Acessos)" : ($type === "sc" ? "SCLogs (Movimentações)" : strtoupper($type)));
  echo "TokenCafe\n";
  ...
  echo "Colunas: data;hora;" . ($includeIp ? "ip;" : "") . "wallet;page;chain;contract;action\n";
  echo "Legenda: data=YYYY-MM-DD; hora=HH:MM:SS; chain=Chain ID; contract=Contrato; action=tipo de ação\n";
  echo "\n";
  echo "data;hora;" . ($includeIp ? "ip;" : "") . "wallet;page;chain;contract;action\n";
  foreach ($paths as $path) {
    $lines = @file($path, FILE_IGNORE_NEW_LINES);
    if (!is_array($lines)) continue;
    foreach ($lines as $ln) {
      ...
      if (!$r) continue;
      if (!match_filters($r, $page, $ip, $wallet, $contract, $hour)) continue;
    echo ($r["date"] ?? "") . ";" . ($r["time"] ?? "") . ";" . ($includeIp ? (($r["ip"] ?? "") . ";") : "") . ($r["wallet"] ?? "") . ";" . ($r["page"] ?? "") . ";" . ($r["chain"] ?? "") . ";" . ($r["contract"] ?? "") . ";" . ($r["action"] ?? "") . "\n";
    }
  }
  exit;
}
...
$baseName = ($type === "report" ? "Report" : ($type === "ip" ? "IPLogs" : ($type === "sc" ? "SCLogs" : $type)));
