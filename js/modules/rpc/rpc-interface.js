...
class RPCInterface {
    ...
    handleNetworkSearch(query) {
        ...
        const filtered = this.popularNetworks.filter(network => 
            network.name.toLowerCase().includes(query.toLowerCase()) ||
            network.chainIdDecimal.toString().includes(query) ||
            ...
            networkInfo.innerHTML = `
                <div class="card">
                    <div class="card-body">
                        ...
                        <button class="btn btn-primary" onclick="rpcInterface.addNetworkToMetaMask()">
            ...
            container.innerHTML = `
                ...
                <div class="card mb-4">
                    ...
                    <div class="card-body">
                        ...
                        <div class="mt-3">
                            <button type="button" class="btn btn-primary" onclick="rpcInterface.addCustomRpc()">
    ...
    generateRpcList(availableRpcs, metamaskRpcs, chainId) {
        ...
        return availableRpcs.map(rpc => {
            // Verifica se este RPC já está no MetaMask
            const isInMetaMask = metamaskRpcs.some(mmRpc => 
            ...
            const buttonAction = isInMetaMask ? 
                `rpcInterface.removeRpcFromMetaMask('${chainId}', '${rpc.url}', '${rpc.name}')` :
                `rpcInterface.addRpcToMetaMask('${chainId}', '${rpc.url}', '${rpc.name}')`;
    ...
    updateRpcButton(rpcUrl, isInMetaMask, rpcName) {
        ...
        const buttonAction = isInMetaMask ? 
            `rpcInterface.removeRpcFromMetaMask('${chainId}', '${rpcUrl}', '${rpcName}')` :
            `rpcInterface.addRpcToMetaMask('${chainId}', '${rpcUrl}', '${rpcName}')`;
