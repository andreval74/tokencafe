...
async function unusedResolveRpcsForCurrentNetwork() {
  try {
    ...
    return {
      rpcs: Array.from(new Set(rpcs)).slice(0, 5),
      chainIdDec: chainIdDec || null,
    };
  } catch {
...
async function runAllTests() {
  ...
  if (!rpcs.length || !chainIdDec) {
    addRow({
      test: "Inicialização",
      rpc: "-",
      detail: "Carteira não conectada ou rede sem RPCs",
      result: "Abortado",
      ok: false,
    });
    updateSummary(1, 0);
  ...
  for (const url of rpcs) {
    ...
    const hRow = {
      test: "[Health] eth_chainId",
      rpc: url,
      detail: `Esperado=${chainIdDec} • Obtido=${h.got ?? "n/a"} • Latência=${h.latencyMs}ms`,
      result: h.got ?? "n/a",
      ok: h.ok,
    };
    if (h.ok) passed++;
    ...
    const bRow = {
      test: "[Block] eth_blockNumber",
      rpc: url,
      detail: `Obtido=${b.got ?? "n/a"}`,
      result: b.got ?? "n/a",
      ok: b.ok,
    };
    if (b.ok) passed++;
    ...
    const nRow = {
      test: "[Net] net_version",
      rpc: url,
      detail: `Obtido=${n.got ?? "n/a"}`,
      result: n.got ?? "n/a",
      ok: n.ok,
    };
    if (n.ok) passed++;
...
async function unusedOptionalDisconnect() {
  try {
    if (window.walletConnector && typeof window.walletConnector.disconnect === "function") {
      await window.walletConnector.disconnect();
    }
