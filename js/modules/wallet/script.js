  ...
  async getWorkingRpcs(chainId) {
    ...
    return chain.rpc
      .filter((rpc) => {
        ...
        return url && url.startsWith("https://") && !url.includes("${") && !url.includes("API_KEY");
      })
...
class TokenCafeWalletManager {
  ...
  async updateWalletInfo() {
    ...
    try {
      ...
      const chainId = await this.retryRpcCall(
        async () => {
      ...
      const networkVersion = await this.retryRpcCall(
        async () => {
    ...
    this.updateBalanceTimeout = setTimeout(async () => {
      ...
      try {
        ...
        const balance = await this.retryRpcCall(
          ...
          3,
          2000,
        ); // 3 tentativas com 2 segundos de delay
  ...
  isCacheValid() {
    const now = Date.now();
    return this.balanceCache.timestamp > 0 && now - this.balanceCache.timestamp < this.balanceCache.ttl;
  }
   ...
   */
  async addTokenToWallet(tokenData, _walletType = "metamask") {
  ...
  async performNetworkSearch(searchTerm) {
    ...
    try {
      ...
      networkAutocomplete.innerHTML = limitedNetworks
        .map(
          (network) => `
   ...
   */
  showNetworkSections() {
    const sections = ["selected-network-section", "selected-currency-section", "selected-urls-section", "custom-rpc-section", "network-management-section"];
  ...
  initNetworkManagementButtons() {
    ...
    if (customUrlsTextarea) {
      customUrlsTextarea.addEventListener("input", () => {
        const urls = customUrlsTextarea.value
          .split("\n")
          .map((u) => u.trim())
          .filter(Boolean);
  ...
  async handleAddNetwork() {
    try {
      ...
      if (customRpcUrls && customRpcUrls.value.trim()) {
        const customUrls = customRpcUrls.value
          .trim()
          .split("\n")
          .filter((url) => url.trim());
  ...
  async handleTestRpc() {
    try {
      ...
      if (result.success) {
        this.showRpcTestResult(`Conectividade OK! Bloco atual: ${result.blockNumber} | Tempo: ${result.responseTime}ms`, "success");
      } else {
  ...
  handleClearNetwork() {
    // Limpar campos
    const fields = ["networkSearch", "selectedNetworkName", "selectedChainId", "selectedNativeCurrency", "selectedCurrencySymbol", "selectedRpcUrl", "selectedExplorerUrl", "customRpcUrls", "selectedNetworkData"];
   ...
   */
  hideNetworkMessages() {
    const messages = ["networkErrorMessage", "networkSuccessMessage", "rpcTestResult"];
  ...
  async getMetaMaskRpcUrls(_chainIdHex = null) {
    const urls = [];
    try {
      ...
      if (!isMetaMask) {
        ...
        try {
          const mmState = await provider.request({
      ...
      // Filtra e normaliza
      const filtered = urls.filter((u) => typeof u === "string" && /^https?:\/\//i.test(u)).filter((u) => !u.includes("${") && !u.includes("API_KEY"));
  ...
  async updateCustomRpcsDisplay() {
    ...
    try {
      ...
      const normalized = Array.isArray(providerRpcs) ? Array.from(new Set(providerRpcs.map((u) => this.normalizeRpcUrl(u)).filter((u) => typeof u === "string" && /^https?:\/\//i.test(u)))) : [];
