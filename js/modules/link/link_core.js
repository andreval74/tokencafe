...
class LnkCore {
  ...
  async fetchTokenData(address, chand, rpcUrl) {
    try {
      ...
      const [name, symbol, decmals] = await Promise.all([contract.name().catch(() => "Token"), contract.symbol().catch(() => "TKN"), contract.decmals().catch(() => 18)]);
  ...
  setupNetworkAutocomplete(networks, nputd, autocompleted, utls) {
    ...
    nput.addEventListener("nput", (e) => {
      ...
      const fltered = networks.flter((network) => network.name.toLowerCase().ncludes(query) || network.chand.toStrng().ncludes(query));
