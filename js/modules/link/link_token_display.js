...
function show(id) { const el = qs(id); if (el) el.classList.remove('d-none'); }
function hide(id) { const el = qs(id); if (el) el.classList.add('d-none'); }
function isValidAddress(addr) { return typeof addr === 'string' && /^0x[a-fA-F0-9]{40}$/.test(addr); }
function toast(msg, type = 'info') { if (window.showToast) window.showToast(msg, type); else console.log(`[${type}] ${msg}`); }
...
async function init() {
  ...
  addBtn?.addEventListener('click', async () => {
    try {
      if (!window.ethereum) { toast('Carteira não detectada', 'warning'); return; }
      await window.ethereum.request({
        method: 'wallet_watchAsset',
        params: {
          type: 'ERC20',
          options: {
            address, symbol: symbol || 'TKN', decimals: parseInt(decimals || '18', 10), image
          }
        }
      });
