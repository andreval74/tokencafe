...
class WidgetSimple {
        ...
        if (validationStatus) {
            validationStatus.innerHTML = `
                <div class="alert alert-info alert-sm">
                    ...
                    <button class="btn btn-sm btn-primary mt-2" onclick="window.widgetSimple.createSaleForToken('${tokenInfo.address}')">
    ...
    async createSaleForToken(tokenAddress) {
        try {
            ...
            // Criar contrato de venda
            const saleAddress = await this.state.contractDeployer.createSimpleSale(
                tokenAddress,
                pricePerToken,
                walletAddress,
                signer
    ...
    async validateContract(contractAddress, chainId) {
        try {
            ...
            if (code === '0x') {
                return { isValid: false, error: 'Contrato não encontrado neste endereço' };
            }

            // Tentar detectar ABI automaticamente
            let abi = await this.detectContractABI(contractAddress, chainId);
            if (!abi) {
                return { isValid: false, error: 'Não foi possível obter a ABI do contrato' };
            }
            ...
            if (!hasPurchaseFunction) {
                ...
                try {
                    ...
                    if (d !== null) {
                        return { isValid: false, error: 'Endereço é um contrato ERC20/BEP20 sem função de compra. Informe o contrato de venda (TokenSale) ou crie um.' };
                    }
                } catch (_) {}
                return { isValid: false, error: 'Contrato não possui função de compra pública/payable' };
            }
            ...
            return { isValid: false, error: `Erro ao validar contrato: ${error.message}` };
        }
    }
    ...
    getMinimalSaleABI() {
        return [
            {"inputs": [], "name": "saleToken", "outputs": [{"internalType": "address", "name": "", "type": "address"}], "stateMutability": "view", "type": "function"},
            ...
            {"inputs": [], "name": "destinationWallet", "outputs": [{"internalType": "address", "name": "", "type": "address"}], "stateMutability": "view", "type": "function"},
            ...
            {"inputs": [], "name": "minimumPurchase", "outputs": [{"internalType": "uint256", "name": "", "type": "uint256"}], "stateMutability": "view", "type": "function"},
            {"inputs": [], "name": "minAmount", "outputs": [{"internalType": "uint256", "name": "", "type": "uint256"}], "stateMutability": "view", "type": "function"},
            {"inputs": [], "name": "maxPurchase", "outputs": [{"internalType": "uint256", "name": "", "type": "uint256"}], "stateMutability": "view", "type": "function"},
            {"inputs": [], "name": "maximumPurchase", "outputs": [{"internalType": "uint256", "name": "", "type": "uint256"}], "stateMutability": "view", "type": "function"},
            {"inputs": [], "name": "maxAmount", "outputs": [{"internalType": "uint256", "name": "", "type": "uint256"}], "stateMutability": "view", "type": "function"},
            {"inputs": [], "name": "perWalletCap", "outputs": [{"internalType": "uint256", "name": "", "type": "uint256"}], "stateMutability": "view", "type": "function"},
    ...
    populateAutoDetectedFields(params) {
        ...
        if (params.pricePerToken) {
            const tokenPriceInput = document.getElementById('tokenPrice');
            if (tokenPriceInput) { // Sempre atualiza o preço se detectado
    ...
    async showPreview() {
        ...
        if (previewEl) {
            ...
            try { this.state.widgetConfig = config; } catch (_) {}
            try { this.state.activeWidgetContainer = previewEl; } catch (_) {}
            ...
            this.getConnectedWallet().then(addr => {
                if (addr && connectedEl) {
                    const short = addr.substring(0, 6) + '...' + addr.substring(addr.length - 4);
                    connectedEl.innerHTML = `<span class="tc-connected-icon">🟢</span><span class="tc-connected-text">Conectado: ${short}</span>`;
            ...
            try {
                ...
                if (semanticsEl) {
                    ...
                    semanticsEl.addEventListener('change', (e) => {
                        try { this.state.priceSemantics = String(e.target.value || 'auto'); } catch (_) {}
                        try { this.appendDebug(`Semântica selecionada=${this.state.priceSemantics}`); } catch (_) {}
                    });
                }
            } catch (_) {}
    ...
    async handlePreviewBuy(config) {
        try {
            ...
            try { this.state.lastEstimateError = null; } catch (_) {}
                    ...
                    try {
                        await window.ethereum.request({
                            ...
                            params: [{
                                ...
                                nativeCurrency: { name: net.symbol || 'NATIVE', symbol: net.symbol || 'NATIVE', decimals: 18 },
            ...
            try {
                ...
                try { this.appendDebug(`getCode(${saleAddress}) len=${(code||'').length}`); } catch (_) {}
                if (!code || code === '0x') {
                    try { this.appendDebug('Endereço sem bytecode. Provavelmente não é contrato nesta rede.'); } catch (_) {}
                ...
                try { this.appendDebug(`Falha getCode: ${codeErr?.message || codeErr}`); } catch (_) {}
            ...
            try {
                const onChainPrice = await (new ethers.Contract(saleAddress, [{"inputs":[],"name":"bnbPrice","outputs":[{"internalType":"uint256","name":"","type":"uint256"}],"stateMutability":"view","type":"function"}], web3)).bnbPrice();
            ...
            const ERC20_DECIMALS_ABI = [{"inputs":[],"name":"decimals","outputs":[{"internalType":"uint8","name":"","type":"uint8"}],"stateMutability":"view","type":"function"}];
            ...
            try {
                ...
                if (valueEth > 1000) { // limite prático para evitar erro de saldo
            ...
            if (semanticsPref === 'A') {
                ...
                try { this.appendDebug(`Semântica preço=A (forçada), valueWei=${valueWei.toString()}`); } catch (_) {}
            } else if (semanticsPref === 'B') {
                valueWei = valueWeiCandidateB;
                console.log('Semântica de preço forçada: B (por unidade mínima)');
                try { this.appendDebug(`Semântica preço=B (forçada), valueWei=${valueWei.toString()}`); } catch (_) {}
            } else {
                // Auto: tentar A primeiro, depois B
                try {
                    if (requiresArg) {
                        await contract.callStatic[fnName](qtyUnitsBN, { value: valueWeiCandidateA });
                        valueWei = valueWeiCandidateA;
                    } else {
                    ...
                    try { this.appendDebug(`Semântica preço=A, valueWei=${valueWei.toString()}`); } catch (_) {}
                } catch (tryAErr) {
                    try {
                        if (requiresArg) {
                            await contract.callStatic[fnName](qtyUnitsBN, { value: valueWeiCandidateB });
                            valueWei = valueWeiCandidateB;
                        } else {
                        ...
                        try { this.appendDebug(`Semântica preço=B, valueWei=${valueWei.toString()}`); } catch (_) {}
                    } catch (tryBErr) {
                        ...
                        try { this.appendDebug(`Falha callStatic A/B: A=${tryAErr?.message || tryAErr}, B=${tryBErr?.message || tryBErr}`); } catch (_) {}
                        valueWei = valueWeiCandidateA;
                    }
                }
            }
            ...
            try {
                ...
                let gasEstimate;
                if (requiresArg) {
                    try { this.appendDebug('Estimando gas...'); } catch (_) {}
                    gasEstimate = await contract.estimateGas[fnName](qtyUnitsBN, { value: valueWei });
                } else {
                    try { this.appendDebug('Estimando gas...'); } catch (_) {}
                ...
                try { this.appendDebug(`gasEstimate=${gasEstimate.toString()} gasLimit=${gasLimit.toString()}`); } catch (_) {}
                // Enviar com gasLimit calculado
                if (requiresArg) {
                    tx = await contract[fnName](qtyUnitsBN, { value: valueWei, gasLimit });
                } else {
                    tx = await contract[fnName]({ value: valueWei, gasLimit });
                }
                try { this.appendDebug(`tx enviada hash=${tx?.hash || 'n/a'}`); } catch (_) {}
            } catch (estimateErr) {
                console.warn('Estimativa de gas falhou, tentando detectar motivo via callStatic:', estimateErr);
                try { this.state.lastEstimateError = estimateErr; } catch (_) {}
                try { this.appendDebug(`Falha estimativa: code=${estimateErr?.code} msg=${estimateErr?.message}`); } catch (_) {}
                try {
                    if (requiresArg) {
                    ...
                    try { this.appendDebug(`callStatic OK; gas fallback=${fallbackGas.toString()}`); } catch (_) {}
                    if (requiresArg) {
                        tx = await contract[fnName](qtyUnitsBN, { value: valueWei, gasLimit: fallbackGas });
                    } else {
                        tx = await contract[fnName]({ value: valueWei, gasLimit: fallbackGas });
                    }
                    try { this.appendDebug(`tx enviada (fallback) hash=${tx?.hash || 'n/a'}`); } catch (_) {}
                } catch (staticErr) {
                    console.error('Execução simulada reverteu:', staticErr);
                    try { this.appendDebug(`callStatic revert: code=${staticErr?.code} msg=${staticErr?.message}`); } catch (_) {}
            ...
            if (receipt && receipt.status === 1) {
                ...
                try { this.appendDebug(`receipt OK status=${receipt?.status} gasUsed=${receipt?.gasUsed?.toString?.() || receipt?.gasUsed} block=${receipt?.blockNumber}`); } catch (_) {}
                console.log('Transação confirmada:', txHash);
                console.groupEnd();
            } else {
                this.showWarning('Transação enviada, mas sem status de sucesso.');
                try { this.appendDebug('receipt status=0 ou ausente'); } catch (_) {}
                ...
                try {
                    ...
                    const callReq = { to: saleAddress, from: account, data: encoded, value: valueWei };
                    ...
                    try {
                        ...
                        if (decoded) {
                            ...
                            try { this.appendDebug(`post-mortem revert reason: ${decoded}`); } catch (_) {}
                        } else {
                            try { this.appendDebug(`post-mortem sem motivo decodificável; raw=${dataHex?.toString?.() || dataHex}`); } catch (_) {}
                        }
                    } catch (_) {}
                }
                console.groupEnd();
            ...
            this.hideLoading();
            try { this.appendDebug(`Erro geral preview buy: ${err?.message || err}`); } catch (_) {}
            ...
            try {
                ...
                if (statusZero && isCallException) {
                    try { this.appendDebug('Iniciando post-mortem via eth_call (status=0)'); } catch (_) {}
                    const fnName = config.purchase.functionName || 'buy';
                    const abi = this.getMinimalSaleABI();
                    const iface = new ethers.utils.Interface(abi);
                    const requiresArg = !!(abi.find(f => f.name === fnName)?.inputs?.length);
                    ...
                    try {
                        ...
                        try { this.appendDebug('eth_call não reverteu; possivelmente falha de execução ao minerar.'); } catch (_) {}
                    } catch (pmErr) {
                        const dataHex = pmErr?.data || pmErr?.error?.data || pmErr?.receipt?.revertReason || '';
                        const decoded = this.decodeRevertReason(String(dataHex || ''));
                        if (decoded) {
            ...
            combined += isCallException || receiptStatus0
                ? 'Transação revertida pelo contrato (CALL_EXCEPTION). Possíveis causas: valor enviado não cobre o preço/decimais, limites de compra, venda não ativa, ou insuficiência de saldo. '
    ...
    showEmbedCode(result) {
        ...
        const owner = (this.state.widgetConfig && this.state.widgetConfig.owner)
            || (result && result.owner)
            || '';
        const code = (this.state.widgetConfig && this.state.widgetConfig.code)
            || (result && result.code)
            || '';
        ...
        try {
            ...
            if (containerEl) {
                    ...
                    if (connectedEl) {
                        this.getConnectedWallet().then(addr => {
                            if (addr) {
                                ...
                                try { this.updatePurchaseStatusBadge(configForHandlers, addr); } catch (_) {}
                            }
                        }).catch(() => {});
                    }
                ...
                if (owner && code) {
                    ...
                    fetch(jsonUrl, { cache: 'no-store', headers: { 'Accept': 'application/json' } })
                        .then(resp => {
                        ...
                        .then(serverCfg => {
                            ...
                            try { this.state.widgetConfig = serverCfg; } catch (_) {}
                            cfg = serverCfg;
                            containerEl.innerHTML = this.createFinalWidgetHTML(cfg);
                            attachHandlers(cfg);
                        })
