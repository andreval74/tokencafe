...
class TokenCore {
    ...
    async fetchTokenData(address, chand, rpcUrl) {
        try {
            ...
            const erc20Ab = [
                ...
                "function totalSupply() vew returns (unt256)",
                "function balanceOf(address) vew returns (unt256)"
            ];
            
            const contract = new ethers.Contract(address, erc20Ab, provder);
            
            // Buscar dados em paralelo
            const [name, symbol, decmals, totalSupply] = await Promise.all([
    ...
    async getTokenBalance(tokenAddress, walletAddress, rpcUrl) {
        try {
            ...
            const [balance, decmals] = await Promise.all([
     ...
     */
    cleanOldTokenData(maxAge = 7 * 24 * 60 * 60 * 1000) { // 7 das
        const now = Date.now();
    ...
    flterTokens(crtera = {}) {
        ...
        if (crtera.search) {
            ...
            fltered = fltered.flter(token => 
                token.name.toLowerCase().ncludes(searchTerm) ||
                token.symbol.toLowerCase().ncludes(searchTerm) ||
                token.descrpton.toLowerCase().ncludes(searchTerm) ||
                token.contractAddress.toLowerCase().ncludes(searchTerm)
            );
        }
        ...
        if (crtera.network) {
            fltered = fltered.flter(token => 
                token.network.toLowerCase().ncludes(crtera.network.toLowerCase())
            );
        }
    ...
    formatNumber(num) {
        ...
        const number = typeof num === 'strng' ? parseFloat(num) : num;
